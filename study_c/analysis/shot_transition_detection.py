...
# api.html
import pandas as pd
from scenedetect import SceneManager, VideoManager, frame_timecode
...
def get_scenes_within_timestamps(scenes, timestamps):
    ...
    for i in range(len(timestamps)):
        if type(timestamps.iloc[i]['ts_perf_start']) != str or \
                type(timestamps.iloc[i]['ts_perf_end']) != str:
            selected_frames.append((frame_timecode
                                    .FrameTimecode
                                    (timecode='00:00:00', fps=fps),
                                    frame_timecode
                                    .FrameTimecode
                                    (timecode='00:00:00', fps=fps)))
        else:
            selected_frames.append((frame_timecode
                                    .FrameTimecode
                                    (timecode=timestamps.ts_perf_start[i],
                                     fps=fps),
                                    frame_timecode
                                    .FrameTimecode(timecode=timestamps
                                                   .ts_perf_end[i], fps=fps)))

    scenes_within_timestamps = []
    scenes_teams = []
    for i in range(len(scenes)):
        for j in range(len(selected_frames)):
            if lies_within_span(scenes[i][0], scenes[i][1],
                                selected_frames[j][0], selected_frames[j][1]):
                ...
                scenes_teams.append([len(scenes_within_timestamps)-1,
                                     ...
                                     scenes[i][0].get_frames(),
                                     scenes[i][1].get_frames(),
                                     timestamps.team[j]])
